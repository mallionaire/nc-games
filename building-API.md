...
You should have separate tables for `categories`, `reviews`, `users` and `comments`. You will need to think carefully about the order in which you create your migrations. You should think carefully about whether you require any constraints on your table columns (e.g. 'NOT NULL')
...
- an review object, which should have the following properties:
  ...
  - `created_at`
  - `votes`
  - `comment_count` which is the total count of all the comments with this review_id - you should make use of knex queries in order to achieve this
...
- `order`, which can be set to `asc` or `desc` for ascending or descending (defaults to descending)
- `owner`, which filters the reviews by the username value specified in the query
